...
/*
 * 石志鹏2018/10/16修改内容（6行）：
 * 164行：如果弹框类型是1则添加全屏滚动效果层
 * 165行：判读如果是type==1则添加一层滚动层<div>，并且下一级自己添加margin：50px auto;
 * 168行：如果type==1添加一层滚动层</div>
 * 181行：原本是!0，让滚动内部的弹框为非定位
 * 314行：显示滚动弹框
 * 446行：原本r<2，增加一次循环多去掉一层父级，去掉父级滚动层
 * 
 */
! function(e, t) {
	"use strict";
	var i, n, a = e.layui && layui.define,
		...
		r = {
			...
			showLoading: function() {
				$('.offerPlusLoadingShade').show();
			},
			closeLoading: function() {
				$('.offerPlusLoadingShade').hide();
			},
			showError: function() {
				$('#offerPlusMsgId').attr('src', 'img/Exception.png');
				$('.offerPlusMsgShade').fadeIn();
				setTimeout(function() {
					$('.offerPlusMsgShade').fadeOut();
				}, 2000)
			},
			tips: function(e, t, n) {
				return r.open(i.extend({
	...
	s.anim = ["layer-anim", "layer-anim-01", "layer-anim-02", "layer-anim-03", "layer-anim-04", "layer-anim-05", "layer-anim-06"], l.pt.config = {
		...
		var n = this,
            ...
            m = '<div class="layer_msk" style="display:none; position: fixed; overflow-y: scroll; top: 0; left: 0; width: 100%; height: 100%; z-index: '+ l +';">'; //石志鹏修改：如果弹框类型是1则添加全屏滚动效果
        return r.zIndex = l, t([r.shade ? '<div class="layui-layer-shade" id="layui-layer-shade' + a + '" times="' + a + '" style="' + ("z-index:" + (l - 1) + "; background-color:" + (r.shade[1] || "#000") + "; opacity:" + (r.shade[0] || r.shade) + "; filter:alpha(opacity=" + (100 * r.shade[0] || 100 * r.shade) + ");") + '"></div>' : "", (n.config.type==1?m:'')+'<div class="' + s[0] + (" layui-layer-" + o.type[r.type]) + (0 != r.type && 2 != r.type || r.shade ? "" : " layui-layer-border") + " " + (r.skin || "") + '" id="' + s[0] + a + '" type="' + o.type[r.type] + '" times="' + a + '" showtime="' + r.time + '" conType="' + (e ? "object" : "string") + '" style="'+ (n.config.type==1?'margin: 50px auto;':'') +'z-index: ' + l + "; width:" + r.area[0] + ";height:" + r.area[1] + (r.fixed ? "" : ";position:initial;") + '">' + (e && 2 != r.type ? "" : d) + '<div id="' + (r.id || "") + '" class="layui-layer-content' + (0 == r.type && r.icon !== -1 ? " layui-layer-padding" : "") + (3 == r.type ? " layui-layer-loading" + r.icon : "") + '">' + (0 == r.type && r.icon !== -1 ? '<i class="layui-layer-ico layui-layer-ico' + r.icon + '"></i>' : "") + (1 == r.type && e ? "" : r.content || "") + '</div><span class="layui-layer-setwin">' + function() {
		...
		}() + "</span>" + (n.config.type == 1?'</div>':'') + (r.btn ? function() {
			var e = "";
		...
		var e = this,
			...
			f = "object" == typeof l,
			c = i("body");
	    if(t.type == 1) t.fixed = (t.fixed == 'yes')?true:0; //石志鹏修改：原本是!0，让滚动内部的弹框为非定位
		if(!i("#" + t.id)[0]) {
	    ...
	    if(this.config.type == 1) this.config.content?this.config.content.closest('.layer_msk').show():'';  //石志鹏修改：展示滚动弹框
		function e() {
			...
			var l = "layui-layer-wrap",
				f = function() {
					if(n === o.type[1] && "object" === t.attr("conType")) {
						...
						for(var a = t.find("." + l), r = 0; r < 3; r++) a.unwrap();   //石志鹏修改：原本r<2，增加一次循环多去掉一层父级，去掉父级滚动层
