...
var SimpleCalendar = function() {
	...
	_createClass(SimpleCalendar, [{
		...
		value: function create() {
			...
			header.innerHTML = header.innerHTML + '<div class="sc-actions goback"><span class="sc-return-today ">返回今天</span></div>';
			...
			for(var i = 0; i < 42; i++) {
				days.innerHTML = days.innerHTML + '<div class="sc-item"><div class="day"></div><div class="lunar-day"></div><span class="time_dateadd"><span>+</span></span></div>';
			}
			//添加下拉框数据
		...
		value: function addData(year, month) {
			...
			for(var i = 0; i < 42; i++) {
				...
				if(this.tyear == writeyear && this.tday == writeday && this.tmonth == writemonth) {
					...
					daysElement[i].classList.add("zj-today");
				}
			}
		...
		key: 'goDate',
		value: function goDate(year, month) {
				var calendar = this;
				var container = this.container;
				var selectYear = container.querySelector('.sc-select-year');
				var selectMonth = container.querySelector('.sc-select-month');
...
SimpleCalendar.prototype.languageData = {
	...
	days_CH: ["星期一", "星期二", "星期三", "星期四", "星期五", "星期六", "星期日"],
