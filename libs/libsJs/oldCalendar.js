...
(function($, window, document, undefined) {
    ...
    Calendar.prototype = {
        ...
        showCalendar: function(refreshData) { // 输入数据并显示
            var self = this;
            var year = dateObj.getDate().getFullYear();
            var month = dateObj.getDate().getMonth() + 1;
            var dateStr = returnDateStr(dateObj.getDate());
            var firstDay = new Date(year, month - 1, 1); // 当前月的第一天
            if (this.opts.monthYearChange && typeof this.opts.monthYearChange == "function" && refreshData != false) {
                this.opts.monthYearChange(year, month, function(obj) {
                    self.opts.dataList = obj;
            ...
            this.$calendarDate_item.each(function(i) {
                ...
                $(this).attr('data', allDay_str).children("span").text(allDay.getDate());
                ...
                // 新加的代码在这里
                if(self.opts.dataList.length>0){
                    for (var j = 0; j < self.opts.dataList.length; j++) {
                        if (self.opts.dataList[j].timeStr === allDay_str) {
                            $(this).addClass('marks');
                        }
                    }
                }
        ...
        renderDOM: function() { // 渲染DOM
            ...
            for (var i = 0; i < 6; i++) {
                _dateStr += '<li class="item"><span>26</span></li>' +
                    '<li class="item"><span>26</span></li>' +
                    '<li class="item"><span>26</span></li>' +
                    '<li class="item"><span>26</span></li>' +
                    '<li class="item"><span>26</span></li>' +
                    '<li class="item"><span>26</span></li>' +
        ...
        inital: function() { // 初始化
            var self = this;
            if (this.opts.isInit) {
                dateObj.setDate(new Date(new Date().getFullYear(), new Date().getMonth(), 1));
            }
            this.renderDOM();
