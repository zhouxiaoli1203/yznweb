...
;!function(){
  ...
  ,Class = function(options){
    ...
    laydate.ready(function(){
        $(that.config.elem).bind({paste: function(){return false;}});
        that.init();
    });
  }
  ...
  */
  ...
  Class.prototype.config = {
    ...
    ,pre_next_month: false //是否开启上个月、下个月，即双控件
  ...
  Class.prototype.init = function(){
    ...
    /*
     *注：2019-06-04 石志鹏修改
     * 由于layer的弹出事件如果是input会绑定在聚焦focus事件上，导致点击事件在聚焦之后执行
     * 会出现focus先执行日历弹出，而click执行时捕获的元素变成了非点击时的元素
     * 导致日历刚弹出就会触发click事件移除元素，出现控件闪烁bug
     * 现将所有的facus事件都初始化为点击click事件。使得日历弹出事件和移除事件同步
     *
     */
  ...
  Class.prototype.render = function(){
    ...
    lay(divFooter).html(function(){
      var html = [], btns = [];
      if(options.range){
          options.btns=["month","week", "clear", "confirm"];
          options.btns.unshift("prevmonth","nextmonth");
      }
      if(options.week){
          options.btns = ["week", "nextweek","clear", "confirm"];
      }
  ...
  Class.prototype.hint = function(content){
    ...
    //加undefined判断，避免异常，undefined原因不知，zmz
    ...
    ,active = {
      ...
      ,confirm: function(){
       ...
       month: function() {
            if (options.range) {
                var myDate = new Date();
                var monthFirst = new Date(myDate.getFullYear(), parseInt(myDate.getMonth()), 1).Format(options.format);
                var totalDay=(new Date(myDate.getFullYear(), parseInt(myDate.getMonth() + 1), 0)).Format(options.format);
            }
            that.setValue(monthFirst+ ' ' + options.range + ' '  +totalDay).remove();
            isStatic && that.calendar();
            that.done();
        },
       prevmonth: function() {
            ...
            var monthFirst = new Date(YM[0], parseInt(YM[1]), 1).Format(options.format);
            var totalDay=(new Date(YM[0], parseInt(YM[1] + 1), 0)).Format(options.format);
            that.setValue(monthFirst+ ' ' + options.range + ' '  +totalDay).remove();
            isStatic && that.calendar();
            that.done();
        },
       nextmonth: function() {
        ...
        week: function() {
            if (options.range) {
                ...
                var day = weekFirstDate.getDay();
                var date = weekFirstDate.getDate();
                var weekEndDate;
                if (day == 0) {
                    weekFirstDate.setDate(date - 6);
                } else {
                    weekFirstDate.setDate(date - day + 1);
                }
                var weekFirst = weekFirstDate.Format(options.format);

                weekEndDate = weekFirstDate;
                weekEndDate.setDate(weekFirstDate.getDate() + 6);
                var weekEnd=weekEndDate.Format(options.format);
            } 
            that.setValue(weekFirst + ' ' + options.range + ' ' + weekEnd).remove();
            isStatic && that.calendar();
            that.done()
        },
        nextweek: function() {
...
//              var weekEnd=weekEndDate.Format(options.format);
//          } 
                var weekFirstDate = that.newDate(dateTime);
                weekFirstDate.setDate(weekFirstDate.getDate() + 7);
                var day = weekFirstDate.getDay();
                var date = weekFirstDate.getDate();
                var weekEndDate;
  ...
  /*
   * TODO 无法复现具体问题
   * 但当laydate和layui同时用amd加载方式加载时laydate的值会被layui里的laydate覆盖，
   * 导致laydate日历控件define返回的laydate对象undefined，
   * 这里去掉layui的加载方式，
   * 具体原因待查
   */
//isLayui ? (
