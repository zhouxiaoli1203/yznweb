...
KindEditor.plugin('image', function(K) {
    var imgkey = "oa_admin_" + new Date().getTime() + "_picture";
    var imgurl = '';
	...
	self.plugin.imageDialog = function(options) {
		...
		uploadbutton.fileBox.change(function(e) {
            ...
            imgFile.readAsDataURL(uploadbutton.fileBox[0].files[0]);
            imgFile.onload = function(){  
                var imgData = this.result; //base64数据
                upLoadQIniu(imgData);
            }
            
            function upLoadQIniu(data) {
                var pic = data.replace(/^.*?,/, '');
                var xhr = new XMLHttpRequest();
                var key = "oa_" + new Date().getTime() + "_picture";
                ...
                xhr.onreadystatechange = function() {
                    if(xhr.readyState == 4) {
                        ...
                        sourceLink = res.key;
                        imgurl = IMGCONFIG.PREFIX_URL + sourceLink;
                        console.log(imgurl)  //打印出图片地址
                        localUrlBox.val(imgurl);
                    }
                }
                xhr.open("POST", url, true);
                xhr.setRequestHeader("Content-Type", "application/octet-stream");
                xhr.setRequestHeader("Authorization", "UpToken " + getQINIUToken().token);
                xhr.send(pic);
            }
            
            function getQINIUToken(call) {
                var token = {};
                $.hello({
