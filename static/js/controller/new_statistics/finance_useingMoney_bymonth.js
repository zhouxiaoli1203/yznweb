define(["datePicker", "echarts", "laydate", "pagination", "angular", "mySelect", "timeDaterangepicker"], function(datePicker, echarts, laydate) {
    return ['$scope', '$rootScope', '$http', '$state', '$stateParams','$timeout', function($scope, $rootScope, $http, $state, $stateParams,$timeout) {
        ...
        var $course = undefined,orderType = 'desc',orderName="price";
        $scope.screen_course = [];
        init();
        function init(){
            $scope.teacListThead = [
                {'name': '课程','width': '30%'},
                {'name': '剩余购买天数','width': '30%','issort': true,'id':'num'},
                {'name': '剩余赠送天数','width': '30%','issort': true,'id':'giveNum'},
        ...
        function onReset(){
         ...
         function switchStudNav(n){
            switch (n){
                case 1:$state.go("financeStatistics/useing_money_bytime");
                    break;
                case 2:$state.go("financeStatistics/useing_money_bymonth");
                    break;
                default:
                    break;
            }
