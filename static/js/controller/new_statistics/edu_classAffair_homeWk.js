define(["datePicker", "echarts", "laydate", "pagination", "angular", "mySelect", "timeDaterangepicker"], function(datePicker, echarts, laydate) {
    return ['$scope', '$rootScope', '$http', '$state', '$stateParams','$timeout', function($scope, $rootScope, $http, $state, $stateParams,$timeout) {
        ...
        var orderType = 'desc',orderName="taskUpRate";
        $scope.compareType1 = true;
        $scope.compareType2 = false;
        $scope.compare = $scope.compare1 = false;
        ...
        function init(){
            ...
            $scope.$watch('tableName',function(){
                $scope.teacListThead = [
                    ...
                    {'name': '布置次数','width': '11%','issort': true,'id':'workTaskTotal'},
                    {'name': '到课人数','width': '11%','issort': true,'id':'studentTotal'},
                    {'name': '提交人数','width': '11%','issort': true,'id':'taskReviewTotal'},
                    {'name': '提交率','width': '11%','issort': true,'sort':'desc','id':'taskUpRate'},
                    {'name': '点评数','width': '11%','issort': true,'id':'reviewRate'},
                    {'name': '点评率','width': '11%','issort': true,'id':'taskReviewRate'},
                    {'name': '家长回复数','width': '11%','issort': true,'id':'reviewsRecordUserTotal'},
                    {'name': '老师回复数','width': '11%','issort': true,'id':'reviewsRecordTeacherTotal'},
                ];
            });
            ...
            $scope.chargeType1 = chargeType1;//一对一/一对多
        ...
        function tab_course_history(n){
            ...
            $scope.activityStatus = undefined;
            switch (n){
                 case 1:$scope.tableName = '老师';
                    break;
                case 2:$scope.tableName = '班级';
                    break;
                default:
                    break;
            }
        ...
        function chargeType1(status){
            if(status){
                if($scope.compare){
                    $scope.compare = true;
                    $scope.compare1 = false;
                    $scope.activityStatus = undefined;
                }
            }else{
                if($scope.compare1){
        ...
        function chargeType(type){
            if(type == 1){
                ...
                if($scope.compareType2){
                    $scope.compareType1 = false;
                    $scope.compareType2 = true;
                    orderName = "taskReviewRate";
                    orderType = "desc";
                    $scope.tableList =  getNewTable(orderName,orderType,$scope.tableList);
                    reChangeChart($scope.tableList);
                }else{
                     $scope.compareType2 = true;
                }
            }
            reChangeChart($scope.chartList);
        }
        function changeData(){
            changeChart();
            changeTable();
        ...
        function changeChart(){
            var params = {
                teachingMethod:$scope.compare?"1":$scope.compare1?"2":undefined,
                dataType:$scope.isactive==1?"teacher":$scope.isactive == 2?"class":"",
            ...
            $.hello({
                url: CONFIG.URL + "/api/oa/statistics/getTaskTableNew",
                type: "get",
                data: params,
                success: function(data) {
                    if (data.status == '200') {
                        $scope.chartList = data.context;
                        reChangeChart(data.context);
                    }
                }

            })
        }
        function reChangeChart(datas){
            var chartData = {
                    contain : $('#mainechart'),
                    dataYname:$scope.compareType1?"作业提交率":$scope.compareType2?"作业点评率":"",
                    dataAxis:[],
                    dataseries:[],
                    times:0
                }
                chartData.times = datas.length;
                if($scope.compareType1){
                    for (var i = 0,len = chartData.times; i < len; i++) {
                        chartData.dataAxis.push(datas[i].name);
                        chartData.dataseries.push(datas[i].taskUpRate);
                    }
                }
                if($scope.compareType2){
                   for (var i = 0,len = chartData.times; i < len; i++) {
                        chartData.dataAxis.push(datas[i].name);
                        chartData.dataseries.push(datas[i].taskReviewRate);
                    } 
                }
...
//                  min: 0,
            ...
            $.hello({
                ...
                success: function(data) {
                    if (data.status == '200') {
                        ...
                        var obj={"workTaskTotal_total":0,"studentTotal_total":0,"taskStudentTotal_total":0,"taskReviewTotal_total":0};
                        angular.forEach(data.context,function(v){
                            obj.workTaskTotal_total+=v.workTaskTotal;
                            obj.studentTotal_total+=v.studentTotal;
                            obj.taskStudentTotal_total+=v.taskStudentTotal;
                            obj.taskReviewTotal_total+=v.taskReviewTotal;
                        });
                        $scope.totalData = obj;
                    }
                }
        ...
        function export_config() {
            ...
            window.open(CONFIG.URL + '/api/oa/statistics/consultantTaskNew?' + $.param(params));
        }
        //返回
