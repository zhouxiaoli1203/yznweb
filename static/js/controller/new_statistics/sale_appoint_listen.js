define(["datePicker", "echarts", "laydate", "pagination", "angular", "mySelect", "timeDaterangepicker"], function(datePicker, echarts, laydate) {
    return ['$scope', '$rootScope', '$http', '$state', '$stateParams','$timeout', function($scope, $rootScope, $http, $state, $stateParams,$timeout) {
        ...
        var $adviser = undefined,orderType = 'desc',orderName="auditionAppointTotal";
        var $type = 0;
        init();
        function init(){
...
//          0 试听预约人数 1试听预约人次 2 试听到课人数 3 试听到课人次 4 试听成单人数
            ...
            $scope.teacListThead = [
                {'name': '顾问','width': '12%',},
                {'name': '预约人数','width': '10%','issort': true,'sort':'desc',id:"auditionAppointTotal",rightBorder:true},
                {'name': '应到','width': '10%','issort': true,id:"auditionToClassTotal"},
                ...
                {'name': '成单率','width': '9%','issort': true,id:"auditionContractRate"},
                {'name': '预约人次','width': '10%','issort': true,id:"auditionAppointTimes",rightBorder:true},
                {'name': '应到','width': '10%','issort': true,id:"auditionToClassTimes"},
            ...
            screen_setDefaultField($scope, function() {
                $scope.screen_goReset['class_type']('预约人数');
            })
            changeData();
        }
...
//                  min: 0,
        ...
        function changeTotal(){
            $.hello({
                url: CONFIG.URL + "/api/oa/statistics/getAuditionTotalNew",
                type: "get",
                success: function(data) {
                    if (data.status == '200') {
