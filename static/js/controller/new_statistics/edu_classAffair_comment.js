define(["datePicker", "echarts", "laydate", "pagination", "angular", "mySelect", "timeDaterangepicker"], function(datePicker, echarts, laydate) {
    return ['$scope', '$rootScope', '$http', '$state', '$stateParams','$timeout', function($scope, $rootScope, $http, $state, $stateParams,$timeout) {
        ...
        var orderType = 'desc',orderName="reviewRate";
        ...
        function init(){
            ...
            $scope.$watch('tableName',function(){
                $scope.teacListThead = [
                    {'name': $scope.tableName,'width': '15%'},
                    {'name': '课堂点评数','width': '17%','issort': true,'id':'reviewTotal'},
                    {'name': '到课人数','width': '17%','issort': true,'id':'studentTotal'},
                    {'name': '课堂点评率','width': '17%','issort': true,'sort':'desc','id':'reviewRate'},
                    {'name': '家长回复数','width': '17%','issort': true,'id':'reviewsRecordUserTotal'},
                    {'name': '老师回复数','width': '17%','issort': true,'id':'reviewsRecordTeacherTotal'},
                ];
            });
        ...
        function changeChart(){
            ...
            $.hello({
                url: CONFIG.URL + "/api/oa/statistics/getReviewTableNew",
                type: "get",
                data: params,
                success: function(data) {
                    if (data.status == '200') {
                        ...
                        for (var i = 0,len = chartData.times; i < len; i++) {
                            ...
                            chartData.dataseries.push(data.context[i].reviewRate);
                        }
...
//                  min: 0,
            ...
            $.hello({
                ...
                success: function(data) {
                    if (data.status == '200') {
                        ...
                        var obj={"reviewTotal_total":0,"studentTotal_total":0,"reviewsRecordUserTotal_total":0,"reviewsRecordTeacherTotal_total":0};
                        angular.forEach(data.context,function(v){
                            obj.reviewTotal_total+=v.reviewTotal;
                            obj.studentTotal_total+=v.studentTotal;
                            obj.reviewsRecordUserTotal_total+=v.reviewsRecordUserTotal;
                            obj.reviewsRecordTeacherTotal_total+=v.reviewsRecordTeacherTotal;
                        });
                        $scope.totalData = obj;
                    }
                }
        ...
        function export_config() {
            ...
            window.open(CONFIG.URL + '/api/oa/statistics/consultantReviewNew?' + $.param(params));
        }
        //返回
