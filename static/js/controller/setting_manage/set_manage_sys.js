define(["socketIo", "equipPop", "qrcode"], function(socketIo) {
    return ['$scope', '$rootScope', '$http', '$state', '$stateParams', 'SERVICE', '$interval', function($scope, $rootScope, $http, $state, $stateParams, SERVICE, $interval) {
        var pay_pop;
        var second = 60,
            timerHandler, isDuringTime = false; //判断验证码定时器是否有效
        init();
        ...
        function init() {
            ...
            $scope.switchVisitNav = switchVisitNav; //切换tab
            $scope.closePop = function() {
                layer.close(dialog);
            }
            $scope.updateShopSupportStatus = function(value) {
                var confirm = layer.confirm(value == 0 ? '是否申请技术支持？申请后“易知鸟售后人员”方可进入校区并协助您解决问题。' : '是否关闭技术支持？关闭后“易知鸟售后人员”将无法登录您的系统。', {
                    title: "确认信息",
                    ...
                    $.hello({
                        url: CONFIG.URL + '/api/oa/shop/updateShopSupportStatus',
                        type: "post",
                        data: JSON.stringify({
                            shopSupportStatus: value == 1 ? 0 : 1
                        }),
