define(["datePicker", "echarts", "laydate", "pagination", "angular", "mySelect", "timeDaterangepicker"], function(datePicker, echarts, laydate) {
    return ['$scope', '$rootScope', '$http', '$state', '$stateParams','$timeout', function($scope, $rootScope, $http, $state, $stateParams,$timeout) {
        ...
        $scope.detailDate = GetDateStr(-1);
        var province, city, shopId = undefined;
        var orderType = 'desc',orderName="studentTotal",defaultData;
        var $type = 0;
        init();
        function init(){
            initCity();
            $scope.teacListThead = [
                {'name': '校区名称','width': '50%','align':'center',},
                {'name': '在读人数','width': '50%','issort': true,'sort':'desc','align':'center','isAlgin':true,'id':'studentTotal'},
            ];
...
//          }
            ...
            if($scope.searchTime){
                var beginTime = $scope.searchTime.split(" 到 ")[0];
                var endTime = $scope.searchTime.split(" 到 ")[1];
                if (beginTime > endTime) {
                    return;
                }
                params["beginTime"] = beginTime+" 00:00:00";
...
//                  min: 0,
        ...
        function changeTable(){
            var params = {
                ...
                city: city,
                'shopId':shopId,
                'beginTime':$scope.detailDate+" 00:00:00",
                'endTime':$scope.detailDate+" 23:59:59",
            };
            $.hello({
                url: CONFIG.URL + "/api/oa/statistics/getShopStudentTotalTableNew",
                type: "get",
                data: params,
                success: function(data) {
                    if (data.status == '200') {
        ...
        function export_config() {
            ...
            window.open(CONFIG.URL + '/api/oa/statistics/consultantShopStudentTotal?' + $.param(params));
        }
        //返回
