define(["datePicker", "echarts", "laydate", "pagination", "angular", "mySelect", "timeDaterangepicker"], function(datePicker, echarts, laydate) {
    ...
    return ['$scope', '$rootScope', '$http', '$state', '$stateParams', function($scope, $rootScope, $http, $state, $stateParams) {
        ...
        var orderName = 'rosterContractPrice',
            orderType = 'desc';
        var timeType = '0';
        $scope.begin_endTime = GetDateStr(-15) + " 到 " + GetDateStr(-1); //起始时间
        $scope.nameType = 'name'; //柱状图默认为消课课时
...
//          })();
        ...
        function getOrderName(data) {
            switch (data) {
                case 'name':
                    ...
                    break;
                case 'potial':
                    return 'rosterToPotentialCustomer';
                    break;
                case 'complete':
                    return 'rosterToContract';
                    break;
                case 'contactPrice':
                    return 'rosterContractPrice';
                    break;
                default:
                    break;
            }
        ...
        function getShopDayDataList_chart() {
            ...
            if ($scope.conditionType == 1) {
                param['orderName'] = getOrderName($scope.nameType);
                param['orderTyp'] = 'desc';
            }
            if ($scope.begin_endTime) {
                param['beginTime'] = $scope.begin_endTime.split(" 到 ")[0]+" 00:00:00";
                ...
                success: function(data) {
                    if (data.status == "200") {
                        ...
                        for (var i = 0; i < times; i++) {
                            ...
                            switch ($scope.nameType) {
                                case 'name':
                                    dataseries.push(data.context[i].addRoster);
                                    break;
                                case 'potial':
                                    dataseries.push(data.context[i].rosterToPotentialCustomer);
                                    break;
                                case 'complete':
                                    dataseries.push(data.context[i].rosterToContract);
                                    break;
                                case 'contactPrice':
                                    dataseries.push(data.context[i].rosterContractPrice);
                                    break;
                                default:
                                    break;
                            }
