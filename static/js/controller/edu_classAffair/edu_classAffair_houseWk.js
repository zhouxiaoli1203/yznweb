define(['laydate', 'SimpleCalendar', "pagination", 'moment', 'timeDaterangepicker', 'mySelect', 'rollCall', 'clsaffairPop', "courseAndClass_sel", "photoPop"], function(laydate) {
    return ['$scope', '$rootScope', '$http', '$state', '$stateParams', function($scope, $rootScope, $http, $state, $stateParams) {
        ...
        $scope.searchTime = "";
        $scope.sel_noExpire = $scope.sel_expire = $scope.sel_nohomeWk = false;
        ...
        function init() {
            ...
            $scope.scheduleListThead = [
                ...
                { 'name': '老师', 'width': '10%' },
                { 'name': '布置时间', 'width': '15%' },
                { 'name': '截止时间', 'width': '15%', 'align': 'center' },
                //              {'name': '状态','width': '80','align': 'center'},
                { 'name': '提交人数', 'width': '80', 'align': 'center' },
                { 'name': '点评人数', 'width': '80', 'align': 'center' },
                { 'name': '未交人数', 'width': '80', 'align': 'center' },
            ];
            ...
            $scope.noExpire = noExpire; //未过期
            $scope.expire = expire; //已过期
            $scope.nohomeWk = nohomeWk; //无作业
            ...
            laydate.render({
                ...
                isRange: true,
                    ...
                    $scope.$emit("clsAffair_houseWk", false);
                }
            });
        ...
        function noExpire() {
            if ($scope.sel_noExpire) {
                $scope.sel_expire = $scope.sel_nohomeWk = false;
            }
            $scope.$emit("clsAffair_houseWk", false);
        }
        ...
        function expire() {
            if ($scope.sel_expire) {
                $scope.sel_noExpire = $scope.sel_nohomeWk = false;
            }
            $scope.$emit("clsAffair_houseWk", false);
        }

        function nohomeWk() {
            if ($scope.sel_nohomeWk) {
                $scope.sel_expire = $scope.sel_noExpire = false;
            }
            $scope.$emit("clsAffair_houseWk", false);
        }
        ...
        $scope.$on("clsAffair_houseWk", function(e, startPage) {
            if (startPage == "true") {
                getListModelInfos(start);
            } else {
                pagerRender = false;
        ...
        function getListModelInfos(start_, fl) {
            ...
            var params = {
                ...
                "classAffairs": $scope.sel_noExpire ? "3" : $scope.sel_expire ? "4" : $scope.sel_nohomeWk ? "5" : undefined,
                "courseListStatus": 3,
            ...
            $.hello({
                ...
                success: function(data) {
                    if (data.status == 200) {
                        ...
                        $scope.scheduleHouseWk = data.context.items;
